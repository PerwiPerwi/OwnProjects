...
SELECT distinct  ksiazka_id FROM recenzja -- a tu niespodzianka o jeden wiecej wiersz!
...
R: 
SELECT klient.*, statystyka_klienta.* FROM (klient JOIN statystyka_klienta USING (klient_id)) JOIN recenzja USING (klient_id)

  
-- ============== 2.3 LEFT/RIGHT/OUTER JOIN
...
-- Wyswietlenie książek z recenzjami (również tych z brakiem recenzji) ale również wyswietlenie recenzji które nie są przypisane do ksiązki (jest to sytuacja błędna)
INSERT INTO  recenzja (	recenzja_id, ksiazka_id, recenzja_tekst, klient_id)  VALUES (8,null,'Test',2); -- to przygotowuje bledne dane
SELECT ksiazka.*, recenzja.*  FROM recenzja FULL JOIN ksiazka USING (ksiazka_id)
DELETE FROM recenzja WHERE ksiazka_id IS NULL;
...
SELECT klient.*, (SELECT liczba_przeczytanych_ksiazek FROM statystyka_klienta WHERE klient_id=2) FROM klient

-- wyswietlenie dane klienta  i w dodatkowej kolumnie wyswietlenie ilosc wystawionych ocen
SELECT klient.*, (SELECT count(*) FROM ocena_ksiazki WHERE klient_id=klient_id) FROM klient

-- ćwiczenia
-- wyświetlić imie,nazwisko autora i tytul a w ostatniej kolumnie ilość wystawionych recenzji (uzyc subquery)
R:
SELECT ksiazka.autor_imie, ksiazka.autor_nazwisko, ksiazka.tytul, (SELECT count(*) FROM recenzja WHERE ksiazka_id=ksiazka_id) FROM ksiazka


-- wyświetlić oceny ksiazki 'Powstanie Warszawskie' (w WHERE ma być wyszukanie po nazwie a nie po ID) w ostatniej kolumnie maksymalną ocenę jaka dostała książka  (zadanie trudniejsze)
-- czyli 'Powstanie Warszawskie', 5, 10 
R:
SELECT k.autor_imie, k.autor_nazwisko, k.tytul, ocena, (SELECT max(ocena) FROM ocena_ksiazki o WHERE o.ksiazka_id=og.ksiazka_id)
FROM ksiazka k JOIN ocena_ksiazki og USING (ksiazka_id) WHERE tytul='Powstanie Warszawskie'
...
-- a jak to posortować żeby były nulle na końcu?
...
) t ORDER BY recenzja_tekst NULLS LAST
