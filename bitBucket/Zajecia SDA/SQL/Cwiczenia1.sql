...
R: SELECT imie, nazwisko FROM klient
...
R: SELECT * FROM ksiazka WHERE autor_imie='Władysław'

- znaleźć książki gdzie imie autora zaczyna się od J
R: SELECT * FROM ksiazka WHERE autor_imie LIKE 'J%'

- wyświetlić wszystkie ksiazki gdzie typy inne niż 'Bajki'
R: SELECT * FROM ksiazka WHERE rodzaj<>'Bajki'
...
-- wyswietlic imiona uzytkownikow ale z pominieciem tych imion które mają autorzy książek
R:
 SELECT imie FROM klient 
 EXCEPT 
 SELECT autor_imie FROM ksiazka


SELECT * FROM ksiazka, recenzja 
